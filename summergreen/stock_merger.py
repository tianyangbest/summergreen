...
class StockMerger(object):
    def __init__(self, parquet_target_dir):
        ...
        self.stock_threading = threading.Thread(
            target=self.stock_dict2tmp_dict, daemon=True
        )
    ...
    def initialize_one_day_job(self, trade_date):
        ...
        self.persistent_cdf = cudf.DataFrame(
            columns=self.ordered_index + self.ordered_columns
        )
    ...
    def stock_dict2tmp_dict(self):
        ...
        while True:
            try:
                print(
                    f"""started cache to tmp_df stock dict at {datetime.datetime.now()}"""
                )
                stock_df_list = [
                    self.tmp_df,
                    pd.DataFrame.from_dict(self.stock_queue.get(), orient="index"),
                ]
                ...
                print(
                    f"""finished cache to tmp_df stock dict at {datetime.datetime.now()}"""
                )
    ...
    def save_persistent2parquet(self):
        ...
        self.persistent_cdf = cudf.concat([self.persistent_cdf, tmp_cdf])
        self.persistent_cdf.to_parquet(
            f"{self.parquet_target_dir}/{self.trade_date}.parquet"
        )
    ...
    def set_merger_plan(self):
        ...
        self.stock_scheduler.add_job(
            self.cache_stock_dict, "cron", hour="10,13-14", max_instances=10, second="*"
        )
        self.stock_scheduler.add_job(
            self.cache_stock_dict,
            "cron",
