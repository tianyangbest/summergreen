...
class Ranger:
    def __init__(self, stock_codes):
        ...
        self.columns_names = list(self.stock_config["sina_pandas_datatype"].keys())
        self.ranger_dict = {
            c: pd.DataFrame(columns=self.columns_names)
            .astype(self.stock_config["sina_pandas_datatype"])
            .set_index("time")
        ...
        for k in self.stock_codes:
            if len(new_dict_ranger_lists[k]) >= 1:
                ...
                new_df = new_df.astype(
                    self.stock_config["sina_pandas_datatype"]
                ).set_index("time")
